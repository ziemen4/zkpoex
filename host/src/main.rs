...
use shared::{
    evm_utils::{get_blockchain_settings, encode_seal},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if eth_rpc_url_str == "http://localhost:8545" {
        shared::log_info!("No onchain verification, using local Ethereum node");
        Ok(())
    } else {
        ...
        println!("Please enter the VerifierContract Ethereum address (starting with 0x) for the current network:");
        io::stdout().flush().unwrap();
        ...
        // send tx to callback function: verifyAndFinalizeVotes 0xA65eb2B6e554550B513568baFb3D07C92fEF8b15
        let address_contract = contract_address_str.trim().parse::<Address>().unwrap();
